...
import { createPackingEntrySchema } from "@/validators/manufacturing";
...
export async function POST(request: NextRequest, { params }: Params) {
  try {
    ...
    const parsed = createPackingEntrySchema.safeParse(body);
    if (!parsed.success) {
      return errorResponse("Validation failed", 400, parsed.error.flatten());
    }
    const lot = await manufacturingService.createPackingEntry(
      id,
    ...
    return await caughtErrorResponse(error, "Failed to create packing entry", 400);
  }
}
