...
import { createGlassEntrySchema } from "@/validators/manufacturing";
...
export async function POST(request: NextRequest, { params }: Params) {
  try {
    ...
    const parsed = createGlassEntrySchema.safeParse(body);
    if (!parsed.success) {
      return errorResponse("Validation failed", 400, parsed.error.flatten());
    }
    const lot = await manufacturingService.createGlassEntry(
      id,
    ...
    return await caughtErrorResponse(error, "Failed to create glass entry", 400);
  }
}
