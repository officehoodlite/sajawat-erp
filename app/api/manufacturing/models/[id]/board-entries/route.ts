...
import { createBoardEntrySchema } from "@/validators/manufacturing";
...
export async function POST(request: NextRequest, { params }: Params) {
  try {
    ...
    const parsed = createBoardEntrySchema.safeParse(body);
    if (!parsed.success) {
      return errorResponse("Validation failed", 400, parsed.error.flatten());
    }
    const lot = await manufacturingService.createBoardEntry(id, parsed.data);
    return successResponse(lot, 201);
  } catch (error) {
    return await caughtErrorResponse(error, "Failed to create board entry", 400);
  }
}
