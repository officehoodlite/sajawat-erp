...
import { createHardwareEntrySchema } from "@/validators/manufacturing";
...
export async function POST(request: NextRequest, { params }: Params) {
  try {
    ...
    const parsed = createHardwareEntrySchema.safeParse(body);
    if (!parsed.success) {
      return errorResponse("Validation failed", 400, parsed.error.flatten());
    }
    const lot = await manufacturingService.createHardwareEntry(
      id,
    ...
    return await caughtErrorResponse(error, "Failed to create hardware entry", 400);
  }
}
